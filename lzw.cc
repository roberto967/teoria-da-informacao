...
namespace {
    ...
    void LZW_Encode(const std::vector<byte>& input, std::vector<byte>& encoded, unsigned maxBits) {
        ...
        std::cout << "TAM INPUT " << size << std::endl;
        ...
        std::cout << "BYTE MAP SIZE " << byteMapSize << std::endl;
        ...
        std::cout << "MAX BITS " << minBits << std::endl;
...
void LZWCodec::decode(const std::vector<byte>& encoded, std::vector<byte>& decoded) {
    decoded.clear();
